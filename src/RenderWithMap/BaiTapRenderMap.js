...
export default class BaiTapRenderMap extends Component {
  ...
  // Viết hàm hiển thị danh sách phim dưới dạng div card bootstrap
  renderFlim = () => {
    const arrFilm = this.dataFilm.map((film, index) => {
      return (
        <div key={index} className="col-3 mt-2">
          <div className="card text-white bg-dark">
            <div style={{background: `url('${film.hinhAnh}') , url('https://picsum.photos/150/450')`, backgroundRepeat:'no-repeat', backgroundPosition: 'center', backgroundSize: 'cover'}}>
              <img
                style={{ opacity: 0 }}
                width="300"
                height="300"
  ...
  render() {
    return (
      <div className="container">
        ...
        <div className="row d-flex">{this.renderFlim()}</div>
      </div>
    );
  }
}
